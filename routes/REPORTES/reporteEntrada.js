...
router.get('/reporte-entradas', verifyToken, async (req, res) => {
    try {
        const user = req.user;
    ...
    `;
        ...
        // Solo filtrar por almacén si NO es admin
        if (user.rol !== 'admin') {
            query += ` WHERE e.id_almacen = ${db.escape(user.almacen_id)} `;
        }
            ...
            <tbody>
                ${entradas.map(e => `
                    <tr>
                        ...
                        ${user.rol === 'admin' ? `<td>${e.usuario}</td>` : ''}
                    </tr>
                `).join('')}
            </tbody>
        </table>
